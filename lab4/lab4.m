...
f = @(t,x1,x2) x1/2 - 2*x2;
g = @(t,x1,x2) 5*x1 - x2;
sol = solvesystem(f,g,0,4*pi,[1 1],0.05);

% The exact solutions are pretty ugly. 
...
exactx = real(a1*exp(r.*t));
exacty = real(a2*exp(r.*t));


plot(sol.x(1,:), sol.x(2,:), exactx, exacty)
xlabel('x1')
ylabel('y1')
legend('heun', 'exact')
...
ff = @(t,x) [f(t,x(1),x(2));g(t,x(1),x(2))];
iode = euler(ff, [1;1], t);
plot(iode(:,1), iode(:,2), sol.x(1,:), sol.x(2,:), exactx, exacty)
legend('euler', 'heun', 'exact')

% With the step size of 0.05, the differences between the methods are
% very small and not very noticable in the graph.
