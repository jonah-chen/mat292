...
x = zeros(101,1); %Preallocate x
for i=1:101 %Set the 101 elements of x to equally spaced points between -3 and 7
    x(i) = -3.1+0.1*i;
end

y = zeros(101,1);
for i=1:101 %Set the values for y
    y(i) = abs(x(i))*x(i);
end
...
%y(t)=exp(-t^2)(t+C), where C is an arbiturary constant

t = linspace(1,5,100); %Creating independent variable t with 100 points
y1 = exp(-t.^2).*(t-exp(1)-1); %y(1)=-1 solution
y2 = exp(-t.^2).*(t-1);        %y(1)= 0 solution
y3 = exp(-t.^2).*(t+exp(1)-1); %y(1)= 1 solution

%Plot 3 solutions on same axes
plot(t,y1,t,y2,t,y3)
%annotating figure
xlabel('t');
ylabel('y');
title('Solutions to dy/dt + (2*t)*y = 1/exp(t^2)');
...
f = @(x) (x.^3-abs(x)).*log(x.^2+1); %Define inline function
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %Definition of function

g = @(y) f(0,y); %computing single solution at x=0
fzero(g,0)

g = @(x) f(x,1/2); %computing two solutions at y=1/2
fzero(g,1/2)
fzero(g,2)
...
% There is one trivial solution that is y=0. 
% The other solutions with y(0)<0 will asymptotically approach y=0 as x
% approaches (+/-)infinity. Solutions with y(0)>0 will approach positive
% infinity as x approaches (+/-)infinity. 
% Solutions are symmetric about the y axis because the direction field
% (more specifically dy/dx) is reflected about the y axis. i.e.
% y'(x)=-y'(-x).
%
% 
%% Exercise 12
...
%The fixed points of the solution are y=-1 (semistable), y=1 (stable), and
%y=2 (unstable). Thus, as x approaches infinity y would exhibit one of the
%following behaviors

%y_0 <= -1       y approaches -1
%-1 < y_0 < 2    y approaches  1
%y_0 = 2         y approaches  2
%y_0 > 2         y approaches infinity
