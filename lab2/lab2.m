...
sol1 = ode45(@(t,y) y.*tan(t)+sin(t), [0, pi], -1/2);

%Exact solution: y(t)=-cos(t)/2
tt = linspace(0,pi,100);
yy = -cos(tt)./2;
plot(tt,yy,sol1.x,sol1.y,'x', 'MarkerSize',10, 'LineWidth', 2)
xlabel('t');
ylabel('y');
legend('Exact', 'Numerical', 'Location',"best");
...
yexact = -cos(sol1.x)./2;
err = abs(yexact - sol1.y);
max(err)
semilogy(sol1.x, err)
xlabel('t');
ylabel('error');

% The error is the largest around x=tau/4 , or around 1.6. At this point,
% y' is at a local maximum. Furthermore, the tan t term that shows up
% approaches infinity at tau/4 (from left). As MATLAB uses floating point 
% numbers rather than exact symbolic numbers, a large number multiplied by 
% a small value (for y) at x close to tau/4 can result in significant 
% rounding error, which likely contributes to the largest error at tau/4.
...
sol4 = ode45(@(t,y) y.^(-2), [1,10], 1); % Inline function & solution

tt = linspace(1,10,100);
yy = (3.*tt-2).^(1/3); % Exact solution

max(abs(yy-deval(sol4,tt))) % Compute max error

% Plo exact & approx solutions
...
sol5 = ode45(@(t,y) 1- t.*y/2, [0,10], -1); %Define and compute solution
...
sol6 = ode45(@(t,y) y.^3 - t.^2, [0,1], 1)

plot(sol6.x, sol6.y)
xlabel('t');
ylabel('y');

% The slopes of the direction field of y' increases at a rapid rate with increasing y. 
% This causes solution will diverge at x=1/2. When MATLAB attempts to
% integrate this to an asymptote, it is not possible to achieve a good
% result. Furthermore, the ode45 solver adjusts its step size depending on
% the difference between two approximations. As MATLAB uses floating point
% numbers, there is a finite precision (in this case ~1.8e-15) that is possible.
% At an asymptote, the approximations cannot meet MATLABs tolerance. Hence,
% it gives a warning and the solution does not make it all the way to t=1.
